...
# Discrete Lagrangian
Ld = h * (m / 2 * ((q1 - q0) / h) ** 2 - k / 2 * ((q1 + q0) / 2) ** 2)

# Compute derivatives
D1_Ld = simplify(diff(Ld, q0))
D2_Ld = simplify(diff(Ld, q1))
D1_Ld = D1_Ld.subs({q0: q1, q1: q2}, simultaneous=True)
DEL_equations = simplify(D1_Ld + D2_Ld)

f_DEL = lambdify([q0, q1, q2, h, m, k], DEL_equations)
...
print("Figure saved as harmonic_oscillator_trajectory.png")
